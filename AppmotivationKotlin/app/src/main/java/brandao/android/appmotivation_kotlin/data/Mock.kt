package brandao.android.appmotivation_kotlin.data
...
class Mock {
   ...
   private val mListPhrase = listOf(
    ...
    fun getPhrase(categoryID: Int, lang: String): Phrase{
        var phrase = mListPhrase[0]
        var numeroSorteado = 1



        val filtered = mListPhrase.filter { (it.categoryID == categoryID || categoryID == all) && it.lang == lang}
        val index = Random.nextInt(filtered.size)
        return filtered[index]

        /*
        Podemos resumir a lógica acima dessa forma
        if (categoryID == 1)
        ...
        filtered[index]
          ...
          when (categoryID){
            ...
            MotivationConstants.FILTER.ALL -> {
                numeroSorteado = Random.nextInt(0, mListPhrase.size)
                print(numeroSorteado.toString())
                phrase = mListPhrase[numeroSorteado]
            ...
            MotivationConstants.FILTER.HAPPY -> {
                ...
                }while (phrase.categoryID != MotivationConstants.FILTER.HAPPY)
            ...
            MotivationConstants.FILTER.SUNNY -> {
                ...
                }while (phrase.categoryID != MotivationConstants.FILTER.SUNNY)
